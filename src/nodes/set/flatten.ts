...
export const mapInput = (input, state) => {
    const { inputs } = sortEntriesNumerically(Object.entries(input)).reduce((acc, [key, value]) => {
...
export const process = (input, state) => {
    const { vals } = input.inputs.flatMap(x => x.value).reduceRight((acc, val: SingleToken) => {
