...
async function writeExamples(examples, name) {
    ...
    const { jsonExamples, tsExamples } = examples.reduce((acc, x) => {
    ...
    await Promise.all((jsonExamples).map(async (x, i) => {
        await fs.outputFile(`./src/components/editor/raw/${name}/${i}.json`, x.value);
    }));
    //Then write the import file 
    await fs.outputFile(`./src/components/editor/nodes/${name}/index.ts`,
        jsonExamples.map((_, i) => `export { default as example${i} } from '@site/src/components/editor/raw/${name}/${i}.json';`).join('\n'))
...
`)
        ...
        x.declaration.children.map(x => {
            ...
            const defaultValue = x.comment?.blockTags?.filter(x => x.tag == '@default')[0]?.content[0].text.slice(TS_PREFIX, -TS_SUFFIX).trim();
            acc.push(`
#### ${x.name}
...
async function generate() {
    ...
    try {
        ...
        const nodes = docsJson.children.filter((x) => x.name.startsWith('nodes')).map(async (x) => {
            ...
            const children = x.children.filter((y) => y.kind == CLASS && y.name == 'default').map(x => x.children.reduce((acc, value) => {
            ...
            return fs.outputFile(`./docs/${x.name}.mdx`,
                `---
