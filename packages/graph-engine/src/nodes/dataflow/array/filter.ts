...
import { DataflowGraph, execute } from '@/capabilities/dataflow.js';
import { DataflowNode } from '@/programmatic/nodes/dataflow.js';
import { Graph } from '../../../graph/graph.js';
import { INodeDefinition } from '../../../programmatic/nodes/node.js';
import { Input, ToInput } from '@/programmatic/dataflow/input.js';
...
export default class ArraySubgraph<T> extends DataflowNode {
	...
	constructor(props: IArraySubgraph) {
		...
		if (!existing) {
			...
			output.dataflow.addInput('matches', {
				type: BooleanSchema,
				visible: true
			});

			input.dataflow.addInput('value', {
				type: AnySchema,
				visible: false,
				annotations: {
					'ui.editable': false,
			...
			input.dataflow.addInput('length', {
				type: NumberSchema,
				visible: false,
				annotations: {
					'ui.editable': false,
	...
	async execute() {
		...
		const output = await (input.value as any[]).reduce(async (acc, item, i) => {
			...
			const result = await execute(this._innerGraph as DataflowGraph, {
