...
import { NodeWithPlayControl } from '@/capabilities/playControls/node.js';
    ...
    register: (graph: Graph) => {
        graph.onFinalize('nodeAdded', node => {
            ...
            if (graph.annotations[annotatedPlayState] === PlayState.PLAYING) {
                (node as NodeWithPlayControl).playControls?.onStart();
            }
            return node;
        });
        ...
        const ctx = {
            ...
            stop: () => {
                ...
                graph.forEachNode(node =>
                    (node as NodeWithPlayControl).playControls?.onStop()
                );
            },
            pause: () => {
                ...
                graph.forEachNode(node =>
                    (node as NodeWithPlayControl).playControls?.onPause()
                );
            },
            resume: () => {
                graph.annotations[annotatedPlayState] = PlayState.PLAYING;
                ...
                graph.forEachNode(node =>
                    (node as NodeWithPlayControl).playControls?.onResume()
                );
            }
