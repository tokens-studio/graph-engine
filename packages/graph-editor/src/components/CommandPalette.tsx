...
const CommandMenu = ({reactFlowWrapper}: {reactFlowWrapper: React.MutableRefObject<HTMLDivElement | null>}) => {
    ...
    React.useEffect(() => {
        const move = (e) => {
            if (wrapperBounds) {
                cursorPositionRef.current = { x: e.clientX - wrapperBounds!.left, y: e.clientY - wrapperBounds!.top };
            } 
        };
    ...
    return (
        <Command.Dialog open={showNodesCmdPalette} onOpenChange={() => dispatch.ui.setShowNodesCmdPalette(!showNodesCmdPalette)} label="Global Command Menu">
            <Box css={{color: '$fgSubtle', display: 'flex', flexDirection: 'row', gap: 1, marginBottom: '$3', padding: '0 $4'}}>
                <SearchIcon />
                        ...
                        .map((item) => (
                            ...
                            >
                                <Stack direction="row" gap={2} align="center">
                                    <Box css={{fontSize: '$xxsmall', color: '$fgSubtle', width: '24px', height: '24px', display: 'flex', alignItems: 'center', justifyContent: 'center'}}>{item.icon}</Box>
                                    {item.text}
                                </Stack> 
                            </Command.Item>
                        ));
                        return <Command.Group heading={key.charAt(0).toUpperCase() + key.slice(1)}>
                            {childValues}
                        </Command.Group>
                    })}
            </Command.List>
