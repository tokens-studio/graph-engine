...
export function useGitHub() {
  ...
  const pushTokensToGitHub = useCallback(
    async (context: GithubCredentials): Promise<RemoteResponseData> => {
      ...
      if (
        ...
        isEqual(
          content.metadata?.tokenSetOrder ?? Object.keys(tokens),
          Object.keys(tokens),
        )
      ) {
      ...
      if (pushSettings) {
        ...
        try {
          ...
          if (
            e instanceof Error &&
            e.message === ErrorMessages.GIT_MULTIFILE_PERMISSION_ERROR
          ) {
            return {
              status: 'failure',
  ...
  const pullTokensFromGitHub = useCallback(
    async (context: GithubCredentials): Promise<RemoteResponseData | null> => {
      const storage = storageClientFactory(context);
  ...
  const syncTokensWithGitHub = useCallback(
    async (context: GithubCredentials): Promise<RemoteResponseData> => {
      try {
        const storage = storageClientFactory(context);
        ...
        if (content) {
          if (
            ...
            if (userDecision) {
              const commitSha = await storage.getCommitSha();
              const sortedValues = applyTokenSetOrder(
                content.tokens,
                content.metadata?.tokenSetOrder,
              );
              saveLastSyncedState(
                dispatch,
                sortedValues,
  ...
  const addNewGitHubCredentials = useCallback(
    async (context: GithubFormValues): Promise<RemoteResponseData> => {
    ...
    [syncTokensWithGitHub, tokens, themes],
  );

  const fetchGithubBranches = useCallback(
    async (context: GithubCredentials) => {
      const storage = storageClientFactory(context);
  ...
  const createGithubBranch = useCallback(
    (context: GithubCredentials, newBranch: string, source?: string) => {
      const storage = storageClientFactory(context);
  ...
  const checkRemoteChangeForGitHub = useCallback(
    async (context: GithubCredentials): Promise<boolean> => {
      const storage = storageClientFactory(context);
      try {
