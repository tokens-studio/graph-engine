...
export class GithubTokenStorage extends GitTokenStorage {
  ...
  public async read(): Promise<
    RemoteGraphStorageFile[] | RemoteGraphStorageErrorMessage
  > {
    try {
      ...
      if (Array.isArray(response.data)) {
        ...
        if (treeResponse && treeResponse.data.tree.length > 0) {
          ...
          return compact(
            jsonFileContents.map<RemoteGraphStorageFile | null>(
              (fileContent, index) => {
                ...
                if (
                  ...
                  const parsed = JSON.parse(
                    fileContent.data as unknown as string,
                  );
