...
const Listener = ({template, channelId}) => {
    ...
    useEffect(() => {
        ...
        const x = setInterval(() => {
            localStorage.setItem(`${PERSISTED_KEY}:${channelId}:${template}`, JSON.stringify(sandpack.files));
        }, 8000)
...
const Editor = () => {
    ...
    const template = (search.get('template') || 'react') as SandpackProviderProps["template"];

    const initialFiles = useMemo(() => {

        const raw = localStorage.getItem(`${PERSISTED_KEY}:${channelId}:${template}`) || '{}'
        return JSON.parse(raw);
    }, [channelId, template]);



    return <div style={{ height: '100%' }}><SandpackProvider template={template} theme="auto" files={{
        ...initialFiles,
        "generated.js": {
            code: 'export default {};',
            readOnly: true
            ...
            <ReflexContainer orientation="vertical">
                ...
                <ReflexElement>
                    <SandpackCodeEditor showRunButton={true}
                        closableTabs={true} showInlineErrors={true} showLineNumbers={true} showTabs={false} />
                </ReflexElement>
                <ReflexSplitter propagate={true} />
                <ReflexElement>
