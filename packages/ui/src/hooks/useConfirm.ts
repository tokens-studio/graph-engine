...
import { confirmStateSelector } from '#/redux/selectors/ui/index.ts';
import { useDispatch } from './useDispatch.ts';
import { ConfirmProps } from '#/redux/models/ui.ts';

export type ResolveCallbackPayload<C = any> =
  | false
  | {
      result: true;
      data: C;
...
function useConfirm<C = any>() {
  ...
  const confirm = useCallback(
    (opts: ConfirmProps) => {
      const { text, description, confirmAction, cancelAction, choices, input } =
        opts;
