...
import { INodeDefinition, StringSchema, ToInput } from "@tokens-studio/graph-engine";
...
export class SeperateNode extends BaseNode {
    ...
    async execute() {
        ...
        await ImageMagick.read(this.cloneImage(image), async  (image: IMagickImage) => {
            await image.separate(ch as Channels, (collection) => {
                collection.at(0).write(image.format,(data) => this.setOutput('image', {
                    data,
                }))
            });
        });
